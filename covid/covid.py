...
class CovidCog(commands.Cog):
    ...
    async def fetch_stats_for_date(self, country, date = None):
        ...
        for r_result in results["results"]:
            if datetime.strptime(r_result["date"], '%Y-%m-%d').date() == day_before_result_date:
        ...
        data = {
            ...
            "world_active_delta": (t_confirmed - t_y_confirmed) - (t_recovered - t_y_recovered) - (t_deceased - t_y_deceased),
            ...
            "country_active_delta": (c_confirmed - c_y_confirmed) - (c_recovered - c_y_recovered) - (c_deceased - c_y_deceased),
            "country_percent_of_world_confirmed": round((float(c_confirmed) / float(t_confirmed)) * 100, 2),
            "country_percent_of_world_recovered": round((float(c_recovered) / float(t_recovered)) * 100, 2),
            "country_percent_of_world_deceased": round((float(c_deceased) / float(t_deceased)) * 100, 2),
            "country_percent_of_world_active": round((float(c_confirmed - c_recovered - c_deceased) / float(t_confirmed - t_recovered - t_deceased)) * 100, 2)
        }
